...
import torch
from torchaudio.functional import lfilter as lf
...
def pre_emphasis(x):
    return lf(x, torch.tensor([1,0]).double(), torch.tensor([1, -hp.preemphasis]).double())


def de_emphasis(x):
    return lf(x, torch.tensor([1, -hp.preemphasis]).double(),torch.tensor([1,0]).double())
