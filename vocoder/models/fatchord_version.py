...
class WaveRNN(nn.Module):
    ...
    def generate(self, mels, batched, target, overlap, mu_law, progress_callback=None):
        ...
        print("Output type: AND GRADIENT_CHECK_10",mels.requires_grad)
        ...
        print("Output type: AND GRADIENT_CHECK_11",mels.requires_grad)
        ...
        output[-20 * self.hop_length:] *= torch.tensor(fade_out)

        print("Output type:",output.shape,output.dtype)
    ...
    def xfade_and_unfold(self, y, target, overlap):
        ...
        y[:, :overlap] *= torch.tensor(fade_in)
        y[:, -overlap:] *= torch.tensor(fade_out)

        unfolded = torch.zeros(total_len).double()
